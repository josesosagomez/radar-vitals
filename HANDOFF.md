...
>
> **The active job is BUILDING M4.** Stage 0 is closed. **Stages 1 + 2 are built and their
> cross-review is OPEN** (`plans/m4_stage12_review.md`) — 14 findings, all now implemented,
> **awaiting Codex's verification pass**. **Stage 3 does not begin until that review closes**
> (plan §7). No user decision is outstanding.
>
...
**Branch `vital_signs_v9c`.** Suite **1572 passed, 0 failed, 0 xfailed**
(`conda run -n radar-vitals python -m pytest tests/ -q`, 2026-07-27); **464** in the two targeted
Stage 1/2 files. v9 UI work remains parked in `git stash@{0}`; relocking/display-holdover stay
reverted.
...
### The Stages 1+2 review — 14 findings, all implemented, none verified

`plans/m4_stage12_review.md` ran five rounds. Codex raised **S12R-01…15** (14 Blocking +
S12R-15 Should-fix); **every behavioural claim was reproduced before being agreed with and not
one finding was rejected.** All now have code. **Codex has not yet verified the result** — that
pass is what the review is waiting on.

Schema is now **version 2**: `SessionDisposition` (`ADMITTED`/`EXCLUDED`/`NO_AGREEMENT`) as one
§6 partition; `RecordKind` (`captured_session` / `pre_capture_attempt`); `checksum_ok` removed in
favour of a derived digest; verification runs **inside** `load_manifest`.

**Two items are deliberately open:**
- **S12R-01 — the §6 item-4 truncation limb is UNIMPLEMENTED and escalated.** §6 names
  `mirror_truncated_bytes` as the mechanism for detecting a mid-recording cut, but
  `LiveFrameSource` sets it to `file_size % bytes_per_frame` — a sub-frame remainder that can
  never locate one. **User decision (2026-07-27): record the conflict, resolve at the M0 freeze.**
  `test_the_item_4_truncation_limb_is_UNIMPLEMENTED_and_escalated` pins the state; delete it only
  with the resolution.
- **The temporal replacement clause** ("no replacement once any of that subject's data is
  scored") is documented with a named **Stage-5** enforcement point in
  `validate_retry_policy`'s docstring. It cannot be decided from a timeless manifest.

**Five rules are DEFINED rather than transcribed** and are flagged in the code for challenge:
validity-map polarity (1-D `bool`, `True` = valid), the `pre_capture_attempt` field boundary,
`_RETRY_REASON_EVIDENCE`, `MethodProvenance`'s shape, and the schema-version bump.
...
### 3.0 Codex's verification pass — **this gates everything**

`plans/m4_stage12_review.md` is OPEN with round 5 posted. Codex writes findings as `S12R-NN`
into `COMMENTS OF CODEX`; process them into `DEBATE COMMENTS` with a **verified** response
(reproduce before agreeing), apply agreed fixes, commit, and poll. The loop ends at
`NO MORE COMMENTS` with every item resolved.

**Nothing is waiting on the user.** The one escalation (S12R-01) has been ruled on.
...
- **M0** — pre-registration freeze + Zenodo deposit (hard gate before any study capture). Waits on M4
  existing (the MOVER *candidate* sensitivity and the M3R-29 primary-CI harness). **S12R-01's
  frozen-content conflict resolves here.**
...
### Working method — earned expensively, twice
- **Read the FROZEN source for what it DECIDES, not for the detail you went looking for.**
  Both this session's worst errors were this: three admission predicates written from the plan's
  list of *causes* rather than §6's definitions, and a test that **asserted the opposite of
  `notes/protocol.md`** because it was read for rates and not for the sentence classifying the
  capture as method development.
- **Fixing a class beats fixing the cited instances.** S12R-10 R2 existed only because round 1
  fixed the four proven examples and left `distance_m` and `frame0_epoch` on permissive `float()`.
- **Mutation testing misses in both directions, demonstrated:** it cannot catch a rule **never
  written** (`capture_config_path`, absent from the schema *and* from the self-referential test
  claiming to check §4 coverage) nor one that **should never have been written** (S12R-13, which
  had a happily passing test asserting the wrong thing). Run it, but never treat a clean sweep as
  evidence the semantics are right.
- **A guard no test can fail on is not a rule.** Three were deleted this session on exactly that
  ground. Prefer deleting speculative surface to defending it.
...
- **`run_config_hash` takes an exact `dict` root** and dispatches on `type(obj)` with **no
  `isinstance` in any encoding path**. NumPy and `pathlib` support were both tried and **removed
  after review** (S0R-07…12). **Do not "helpfully" re-add either.** It is an **exact-run provenance
  key only** — never an estimator-equivalence or grouping key.
- **Exact types everywhere in the manifest, via `type(x) is`, never `isinstance`.** `bool` is a
  subclass of `int` and `isinstance` admits `np.bool_`; `bool("false")` is `True`, which is how a
  FAILED checksum was once ADMITTED.
- **A malformed primitive is a `ManifestError`, never a §6 reason.** Exclusion reasons are reported
  study-wide, so a schema defect logged as a disposition puts a fabricated cause in a published table.
- **`checksum_ok` is GONE.** The digest is derived from bytes on disk. Do not reintroduce a
  manifest-declared integrity fact — the operator would supply both the verdict and its evidence.
- **`NO_AGREEMENT` is derived, never declared:** unbound reference **+** verified acquisition record
  **+** nothing at `reference_expected_path`. A bound-then-missing reference is **LOST** (provenance
  failure), never no-agreement.
- **Verification lives inside `load_manifest`**, not in a helper a caller may skip.
- **The M4 scoring grid is frozen at 20 Hz / 600 frames** and the API rejects anything else. Validate
  arguments, then compute from the **validated** values — validating a coerced copy is S12R-08 R2.

### Frozen analysis decisions
- **The quantile method is `linear`** (user, 2026-07-26, M4R-09). **Pass it explicitly at every call
  site**; never rely on a library default.
...
- **M2 done-when #5 stays OPEN** — it closes only when a capture carrying a persisted `frame0_epoch`
  is scored.
...
- **Never put a number in a document bound for the M0 deposit unless it traces to a committed
  script** (M4R-13) — **and measure it, never derive it by arithmetic** (S12R-15).
...
- **BR reference = Masimo `rr_bpm`, cross-checked against the paced metronome.** RRp is pleth-derived
  and *appears* smoothed and laggy — an **inference**, not a measured device fact (M3R-38).
...
- **Multi-line `python -c` and stdin heredocs under `conda run` silently produce no output.** Write a
  scratch `.py` file and run that.
- **PowerShell `-replace` is CASE-INSENSITIVE by default.** It mangled string literals during a bulk
  rename this session (`"admission"` → `"SessionDisposition"`). Use `-creplace`, or better, do bulk
  edits with a scratch Python script.
- **PowerShell has no heredocs**, and a here-string written with LF will not match a CRLF file. Use
  `-F <file>` for multi-line commit messages.
- **A mutation harness must VERIFY its restore.** A bare `write_text` in a `finally` block failed with
  `OSError [Errno 22]` this session and left a mutant live in `src/m4/manifest.py`. Always re-run the
  baseline and audit for leftover mutant text afterwards.
...
- **`scripts/validate_warmup_selection.py`'s `SESSIONS` expectation table is STALE** (pre-M2: it
  expects bin 23 for `massimo1`, but the current lock is 27).
...
- **`packets_dropped` MAY exceed `packets_received`** — they are not a partition. `n_received`
  counts arriving packets; `n_dropped` counts sequence-gap **sizes**. Rejecting that ratio makes the
  worst-loss sessions unloadable and inflates coverage (S12R-13).
...
| **M4 Stage 1 — manifest schema, verification, disposition** | `src/m4/manifest.py` |
