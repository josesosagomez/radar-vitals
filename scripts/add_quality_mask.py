...
------------------------------------
1. Add an entry under soft_failures in scripts/quality_mask_config.yaml with
...
Run from repo root:
    python -X utf8 scripts/add_quality_mask.py --session exp003
...
LOG_PATH    = REPO_ROOT / "results" / "save_quality_mask.log"
...
def _compute_quality_mask(
    ...
    locked_bin : subject range-bin index from manifest (may be overridden by config).
    ...
    # Soft failure: RX channel imbalance
    ...
    # Soft failure: motion spike (subject-bin energy burst detector)
    ms_cfg = cfg.get("soft_failures", {}).get("motion_spike", {})
    if ms_cfg:
        # Resolve effective locked bin: config override wins over manifest value
        cfg_bin = ms_cfg.get("locked_bin")  # None if yaml null
        effective_bin = cfg_bin if cfg_bin is not None else locked_bin

        skipped      = False
        skip_reason  = None

        if effective_bin is None:
            skipped     = True
            skip_reason = "missing locked_bin (not in manifest and not set in config)"
            print(f"  WARNING: motion_spike check skipped — {skip_reason}")
        else:
            effective_bin = int(effective_bin)
            # For explicit config overrides, raise hard; for manifest values, warn+skip
            n_samples = cube.shape[3]
            is_complex = np.iscomplexobj(cube)
            n_bins = n_samples if is_complex else (n_samples // 2 + 1)
            if not (0 <= effective_bin < n_bins):
                if cfg_bin is not None:
                    raise ValueError(
                        f"motion_spike.locked_bin={cfg_bin} is out of FFT range "
                        f"[0, {n_bins - 1}] for {'complex' if is_complex else 'real'} "
                        f"input with {n_samples} samples"
                    )
                else:
                    skipped     = True
                    skip_reason = (
                        f"manifest locked_bin={effective_bin} is out of FFT range "
                        f"[0, {n_bins - 1}]"
                    )
                    print(f"  WARNING: motion_spike check skipped — {skip_reason}")
                    effective_bin = None
    ...
    # Soft failure: subject-bin SNR
    snr_cfg = cfg.get("soft_failures", {}).get("subject_bin_snr", {})
    if snr_cfg:
        cfg_bin      = snr_cfg.get("locked_bin")
        effective_bin = cfg_bin if cfg_bin is not None else locked_bin
        ...
        if effective_bin is None:
            ...
            if not (0 <= effective_bin < n_bins):
                if cfg_bin is not None:
                    raise ValueError(
                        f"subject_bin_snr.locked_bin={cfg_bin} is out of FFT range "
                        f"[0, {n_bins - 1}]"
                    )
                else:
                    skipped     = True
        ...
        results["snr_locked_bin"]  = effective_bin
        ...
        if not skipped:
            snr_flagged, snr_vals = _flag_subject_bin_snr(
                ...
                chunk_frames     = snr_cfg.get("chunk_frames"),
            )
    ...
    # Soft failure: subject-bin energy dropout
    ...
    if do_cfg:
        ...
        if "subject_bin_energy" in results:
            # Energy already computed by motion_spike — reuse, no second FFT
            energy_source = "reused_subject_bin_energy"
        else:
            # Fallback: must compute energy ourselves
            cfg_bin      = do_cfg.get("locked_bin")
            effective_bin = cfg_bin if cfg_bin is not None else locked_bin

            if effective_bin is None:
                skipped     = True
                skip_reason = (
                    "missing locked_bin (not in manifest and not set in config) "
                    "and no reusable subject_bin_energy"
                )
                ...
                if not (0 <= effective_bin < n_bins):
                    if cfg_bin is not None:
                        raise ValueError(
                            f"subject_bin_dropout.locked_bin={cfg_bin} is out of "
                            f"FFT range [0, {n_bins - 1}]"
                        )
                    else:
                        skipped     = True
                        skip_reason = (
                ...
                if not skipped:
                    energy = _subject_bin_energy(
                        cube, effective_bin,
                        chunk_frames=do_cfg.get("chunk_frames"),
                    )
                    results["subject_bin_energy"] = energy
                    do_locked_bin  = effective_bin
    ...
    # Soft failure: phase jump detector
    pj_cfg = cfg.get("soft_failures", {}).get("phase_jump", {})
    if pj_cfg:
        cfg_bin       = pj_cfg.get("locked_bin")
        effective_bin = cfg_bin if cfg_bin is not None else locked_bin
        ...
        if effective_bin is None:
            ...
            if not (0 <= effective_bin < n_bins):
                if cfg_bin is not None:
                    raise ValueError(
                        f"phase_jump.locked_bin={cfg_bin} is out of FFT range "
                        f"[0, {n_bins - 1}]"
                    )
                else:
                    skipped     = True
        ...
        results["phase_jump_locked_bin"]  = effective_bin
        ...
        if not skipped:
            phase_delta_arr, _ = _subject_bin_phase_delta(
                cube, effective_bin,
                method=method,
                chunk_frames=pj_cfg.get("chunk_frames"),
            )
...
def _write_log(session_id: str, stats: dict, cfg: dict, h5_path: Path) -> None:
    hf  = cfg["hard_failures"]
    now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [
        f"\n[{now}]  {session_id}",
        f"  H5 file          : {h5_path.name}",
        f"  Trim             : {stats['trim_frames']} frames ({stats['trim_s']:.1f} s)",
        f"  Analysis frames  : {stats['n_analysis']} ({stats['analysis_s']:.1f} s)",
        f"  --- Hard failures ---",
        f"  Clipping (threshold={hf['clip_threshold']}) : "
        f"{stats['n_clip']} frames",
        f"  Dead (fraction={hf['dead_frame_fraction']}) : "
        f"{stats['n_dead']} frames",
    ]
    ...
    if ms_cfg:
        ...
        if stats.get("motion_spike_skipped", True):
            ...
            soft_lines.append(
                f"  Motion spike energy ({enabled_str}, factor={factor}, "
    ...
    if pj_cfg:
        ...
        if stats.get("phase_jump_skipped", True):
            ...
            cf_note = ""
            if pj_cfg.get("chunk_frames") is not None:
                cf_note = f", chunk_frames={pj_cfg['chunk_frames']}"
            soft_lines.append(
                f"  Phase jump ({enabled_str}, threshold={thr} rad, "
                f"method={method}, locked_bin={pj_bin}{cf_note}): "
...
def _process_session(session_id: str, cfg: dict, commit: str, locked_bin: int | None = None) -> None:
    h5_path = CUBES_DIR / f"{session_id}.h5"
    if not h5_path.exists():
        raise FileNotFoundError(f"{h5_path} not found — run save_time_domain_cubes.py first")

    trim_frames   = int(cfg["trim_frames"])
    frame_rate_hz = 20.0  # read from h5 attrs below
    ...
    with h5py.File(h5_path, "a") as f:
        ...
        # Load only the analysis portion — avoids loading the trim period
        cube = f["cube"][trim_frames:].astype(np.complex64)

        results = _compute_quality_mask(cube, cfg, locked_bin=locked_bin)
        stats   = _run_stats(results, trim_frames, frame_rate_hz)
        # Carry motion-spike skip state into stats for logging
        if cfg.get("soft_failures", {}).get("motion_spike"):
            stats["motion_spike_skipped"]     = results.get("motion_spike_skipped", True)
            stats["motion_spike_skip_reason"] = results.get("motion_spike_skip_reason")
            stats["motion_spike_locked_bin"]  = results.get("motion_spike_locked_bin")
        if cfg.get("soft_failures", {}).get("subject_bin_snr"):
            stats["snr_skipped"]     = results.get("snr_skipped", True)
            stats["snr_skip_reason"] = results.get("snr_skip_reason")
            stats["snr_locked_bin"]  = results.get("snr_locked_bin")
        if cfg.get("soft_failures", {}).get("subject_bin_dropout"):
            stats["dropout_skipped"]       = results.get("dropout_skipped", True)
            stats["dropout_skip_reason"]   = results.get("dropout_skip_reason")
            stats["dropout_locked_bin"]    = results.get("dropout_locked_bin")
            stats["dropout_energy_source"] = results.get("dropout_energy_source")
        if cfg.get("soft_failures", {}).get("phase_jump"):
            stats["phase_jump_skipped"]     = results.get("phase_jump_skipped", True)
            stats["phase_jump_skip_reason"] = results.get("phase_jump_skip_reason")
            stats["phase_jump_locked_bin"]  = results.get("phase_jump_locked_bin")
            stats["phase_jump_method"]      = results.get("phase_jump_method", "mean_phasor")
        ...
        if ms_cfg:
            ...
            cf = ms_cfg.get("chunk_frames")
            if cf is not None:
                qm.attrs["motion_spike_chunk_frames"] = int(cf)
        snr_cfg = cfg.get("soft_failures", {}).get("subject_bin_snr", {})
        if snr_cfg:
            ...
            snr_cf = snr_cfg.get("chunk_frames")
            if snr_cf is not None:
                qm.attrs["subject_bin_snr_chunk_frames"] = int(snr_cf)
        do_cfg = cfg.get("soft_failures", {}).get("subject_bin_dropout", {})
        if do_cfg:
            ...
            do_cf = do_cfg.get("chunk_frames")
            if do_cf is not None:
                qm.attrs["subject_bin_dropout_chunk_frames"] = int(do_cf)
        pj_cfg = cfg.get("soft_failures", {}).get("phase_jump", {})
        if pj_cfg:
            ...
            pj_cf = pj_cfg.get("chunk_frames")
            if pj_cf is not None:
                qm.attrs["phase_jump_chunk_frames"] = int(pj_cf)
            # Phase-diff percentile stats
        ...
        # Write / overwrite quality_metrics group
        if "quality_metrics" in f:
            del f["quality_metrics"]
        ...
        if "subject_bin_dropout_flagged" in results:
            grp.create_dataset("subject_bin_dropout_flagged",
    ...
    _write_log(session_id, stats, cfg, h5_path)
    print(f"  quality_mask written -> {h5_path.name}")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add per-frame quality mask to time-domain cube HDF5 files"
    )
    ...
    # Build a per-session locked_bin lookup from the manifest.
    # Needed when any soft-failure block that uses locked_bin is present
    # without a config-level override.
    sf = cfg.get("soft_failures", {})
    needs_locked_bin = bool(
        (sf.get("motion_spike") and sf["motion_spike"].get("locked_bin") is None)
        or (sf.get("subject_bin_snr") and sf["subject_bin_snr"].get("locked_bin") is None)
        or (sf.get("subject_bin_dropout") and sf["subject_bin_dropout"].get("locked_bin") is None)
        or (sf.get("phase_jump") and sf["phase_jump"].get("locked_bin") is None)
    )

    def _parse_locked_bin(raw) -> int | None:
        """Parse a raw manifest cell into an int, or None with a warning."""
        if raw is None or (isinstance(raw, float) and np.isnan(raw)):
            return None
        try:
            fval = float(raw)
        except (TypeError, ValueError):
            print(f"  WARNING: cannot parse locked_bin={raw!r} from manifest — skipping")
            return None
        ival = int(fval)
        if fval != ival:
            print(f"  WARNING: non-integer locked_bin={fval} in manifest — skipping")
            return None
        return ival

    locked_bin_map: dict[str, int | None] = {}
    if needs_locked_bin:
        if "locked_bin" not in manifest.columns:
            print("WARNING: manifest has no 'locked_bin' column — motion_spike will be skipped for all sessions")
        else:
            for _, row in manifest.iterrows():
                sid = str(row["session_id"])
                locked_bin_map[sid] = _parse_locked_bin(row.get("locked_bin"))
    ...
    for sid in session_ids:
        print(f"\n{sep}\n  {sid}\n{sep}")
        # Config override wins; manifest value used otherwise
        ms_cfg    = cfg.get("soft_failures", {}).get("motion_spike", {})
        cfg_bin   = ms_cfg.get("locked_bin") if ms_cfg else None
        manifest_bin = locked_bin_map.get(sid) if needs_locked_bin else None
        effective_locked_bin = cfg_bin if cfg_bin is not None else manifest_bin
        try:
            _process_session(sid, cfg, commit, locked_bin=effective_locked_bin)
    ...
    if args.all:
        ...
        print(f"\n{sep}")
        print(f"  Done: {ok}/{len(session_ids)} sessions")
        if failed:
            print(f"  Failed: {[s for s, _ in failed]}")
